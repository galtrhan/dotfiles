...
start_recording() {
    ...
    setsid "${CMD_WF}" -g "${GEOM}" -f "${OUTFILE}" >/dev/null 2>&1 &
    WF_PID=$!
    ...
    notify "Screen capture started" "Recording to: ${OUTFILE}"
    echo "${WF_PID}"
}
    ...
    if exists "${CMD_WL_COPY}"; then
        ...
        if [[ "${copy_ok}" -eq 0 ]]; then
            ...
            if exists "${CMD_CLIPHIST}"; then
                notify "Screen capture finished" "Saved: ${OUTFILE} — copied to clipboard using: ${copy_method} (cliphist should store it)."
            else
                notify "Screen capture finished" "Saved: ${OUTFILE} — copied to clipboard using: ${copy_method}."
            fi
            rm -f "${OUT_FILE_FILE}" 2>/dev/null || true
            return 0
        fi
        ...
        if printf '%s' "${OUTFILE}" | "${CMD_WL_COPY}" >/dev/null 2>&1; then
            notify "Screen capture finished" "Saved: ${OUTFILE} — file path copied to clipboard (you can paste the path)."
            rm -f "${OUT_FILE_FILE}" 2>/dev/null || true
            return 0
        fi
        ...
        if exists "ffmpeg"; then
            ...
            if ffmpeg -y -i "${OUTFILE}" -vf "scale=iw/3:ih/3" -vframes 1 -f image2 "${thumb}" >/dev/null 2>&1; then
                ...
                if "${CMD_WL_COPY}" --type=image/png < "${thumb}" >/dev/null 2>&1 || "${CMD_WL_COPY}" < "${thumb}" >/dev/null 2>&1; then
                    notify "Screen capture finished" "Saved: ${OUTFILE} — thumbnail copied to clipboard."
                    rm -f "${thumb}" 2>/dev/null || true
                    rm -f "${OUT_FILE_FILE}" 2>/dev/null || true
                    return 0
                else
                    notify "Screen capture finished" "Saved: ${OUTFILE} — thumbnail generation succeeded but copying thumbnail failed."
                    rm -f "${thumb}" 2>/dev/null || true
                    rm -f "${OUT_FILE_FILE}" 2>/dev/null || true
                    return 0
                fi
        ...
        notify "Screen capture finished" "Saved: ${OUTFILE} — failed to copy to clipboard. You can find the file in your Videos folder."
        rm -f "${OUT_FILE_FILE}" 2>/dev/null || true
        return 0
    else
        # wl-copy not installed: just notify with file location and offer copying path if wl-copy missing
        if exists "${CMD_CLIPHIST}"; then
            notify "Screen capture finished" "Saved: ${OUTFILE}. cliphist is installed but wl-copy is missing; consider installing wl-clipboard (wl-copy) to copy files to clipboard automatically."
        else
            notify "Screen capture finished" "Saved: ${OUTFILE}."
        fi
        rm -f "${OUT_FILE_FILE}" 2>/dev/null || true
        return 0
    fi
